...
def init_db():
    ...
    """)
    cursor.execute("CREATE INDEX IF NOT EXISTS idx_vtr_tenant ON virtual_tag_rules(tenant_id)")
...
VALID_CONDITION_FIELDS = ("resource_group", "service_name", "subscription_id",
                          "resource_name", "resource_type", "cloud_provider")
VALID_OPERATORS        = ("contains", "equals", "starts_with", "ends_with", "not_contains")


def get_virtual_tag_rules(tenant_id: int, enabled_only: bool = False) -> list:
    conn = get_db()
    q = "SELECT * FROM virtual_tag_rules WHERE tenant_id=?"
    if enabled_only:
        q += " AND enabled=1"
...
def create_virtual_tag_rule(tenant_id: int, tag_key: str, tag_value: str,
                             condition_field: str, condition_operator: str,
                             condition_value: str, cloud: str = "",
                             priority: int = 0) -> int:
    conn = get_db()
    cur = conn.execute(
        "INSERT INTO virtual_tag_rules (tenant_id,tag_key,tag_value,condition_field,"
        "condition_operator,condition_value,cloud,priority) VALUES(?,?,?,?,?,?,?,?)",
        (tenant_id, tag_key.strip(), tag_value.strip(), condition_field,
         condition_operator, condition_value.strip(), cloud.strip().lower(), priority)
    )
...
def update_virtual_tag_rule(rule_id: int, tenant_id: int, **fields):
    allowed = {"tag_key","tag_value","condition_field","condition_operator",
               "condition_value","cloud","priority","enabled"}
    ...
    conn.execute(
        f"UPDATE virtual_tag_rules SET {set_clause} WHERE id=? AND tenant_id=?",
        list(updates.values()) + [rule_id, tenant_id]
    )
    conn.commit()
    conn.close()


def delete_virtual_tag_rule(rule_id: int, tenant_id: int):
    conn = get_db()
    conn.execute("DELETE FROM virtual_tag_rules WHERE id=? AND tenant_id=?",
                 (rule_id, tenant_id))
    conn.commit()
    conn.close()


def _rule_to_sql_condition(rule: dict) -> tuple:
    """Convert a virtual tag rule to a SQL condition fragment and params."""
    ...
    if op == "equals":
        ...
        return (f"LOWER({field}) NOT LIKE LOWER(?)", [f"%{val}%"])
    return None, []


def build_virtual_tag_sql_filter(tag_key: str, tag_value: str,
                                  tenant_id: int, cloud: str = "") -> tuple:
    """Return (sql_fragment, params) to filter cost_data by a virtual tag key:value."""
    rules = get_virtual_tag_rules(tenant_id, enabled_only=True)
    matching = [r for r in rules
                if r["tag_key"].lower() == tag_key.lower()
                and r["tag_value"].lower() == tag_value.lower()
                and (not r["cloud"] or not cloud or r["cloud"] == cloud.lower())]

    if not matching:
        return ("1=0", [])  # No rules → no results

    conditions, params = [], []
    for rule in matching:
        cond, p = _rule_to_sql_condition(rule)
        if cond:
...
def get_distinct_virtual_tag_keys(tenant_id: int) -> list:
    """Return distinct tag keys from virtual rules + existing cost_data tags."""
    conn = get_db()
    # From virtual rules
    rule_keys = [r["tag_key"] for r in
                 conn.execute("SELECT DISTINCT tag_key FROM virtual_tag_rules WHERE tenant_id=? AND enabled=1",
                              (tenant_id,)).fetchall()]
    conn.close()
    return sorted(set(rule_keys))


def get_virtual_tag_values(tenant_id: int, tag_key: str) -> list:
    """Return distinct tag values for a given key from virtual rules."""
    conn = get_db()
    rows = conn.execute(
        "SELECT DISTINCT tag_value FROM virtual_tag_rules WHERE tenant_id=? AND tag_key=? AND enabled=1",
        (tenant_id, tag_key)
    ).fetchall()
    conn.close()
    return sorted(r["tag_value"] for r in rows)


def get_cost_by_virtual_tag(tag_key: str, tenant_id: int,
                             date_from: str, date_to: str,
                             cloud: str = "") -> list:
    """Return cost breakdown grouped by virtual tag values for a given key."""
    rules = get_virtual_tag_rules(tenant_id, enabled_only=True)
    tag_rules = [r for r in rules
                 if r["tag_key"].lower() == tag_key.lower()
                 and (not r["cloud"] or not cloud or r["cloud"] == cloud.lower())]
    ...
    conn = get_db()
    results = []
    cloud_filter = f"AND cloud_provider='{cloud}'" if cloud else ""
    tid_filter   = f"AND tenant_id={tenant_id}"

    # Group by tag_value — for each unique value, sum costs from matching rules
    seen_values = {}
    for rule in tag_rules:
        tv = rule["tag_value"]
        if tv not in seen_values:
            seen_values[tv] = []
        seen_values[tv].append(rule)

    for tag_val, val_rules in seen_values.items():
        conditions, params = [], []
        ...
        where = f"WHERE date>=? AND date<=? {tid_filter} {cloud_filter} AND ({' OR '.join(conditions)})"
        all_params = [date_from, date_to] + params
        ...
        if total > 0:
            results.append({"tag_value": tag_val, "total": total})
