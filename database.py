...
def init_db():
    ...
    # Migration: add schedule_tz to custom_reports if missing
    try:
        cursor.execute("SELECT schedule_tz FROM custom_reports LIMIT 1")
    except Exception:
        try:
    ...
    # ── Atlassian users (per-org user directory captured at each cost sync) ────
    cursor.execute("""
        CREATE TABLE IF NOT EXISTS atlassian_users (
            id INTEGER PRIMARY KEY AUTOINCREMENT,
            tenant_id         INTEGER,
            ...
            name              TEXT,
            email             TEXT,
            status            TEXT,   -- active / inactive / for_deletion / suspended
            membership_status TEXT,
            last_active       TEXT,   -- most recent activity date across products
            products          TEXT,   -- JSON list of product keys
            synced_at         TEXT,
            UNIQUE(tenant_id, org_id, account_id)
        )
    """)
    cursor.execute(
    ...
    # ── Cursor usage events aggregated per (user, model) ──────────────────────
    cursor.execute("""
        CREATE TABLE IF NOT EXISTS cursor_usage (
            ...
            model           TEXT,
            included_cents  REAL DEFAULT 0,
            on_demand_cents REAL DEFAULT 0,
            tokens          INTEGER DEFAULT 0,
            events          INTEGER DEFAULT 0,
            synced_at       TEXT,
            UNIQUE(tenant_id, email, model)
        )
    """)
    ...
    cursor.execute("""
        CREATE TABLE IF NOT EXISTS azure_setup_tokens (
            id INTEGER PRIMARY KEY AUTOINCREMENT,
            token TEXT UNIQUE NOT NULL,
            tenant_id INTEGER NOT NULL,
            status TEXT DEFAULT 'pending',
    ...
    cursor.execute("""
        CREATE TABLE IF NOT EXISTS gcp_setup_tokens (
            id INTEGER PRIMARY KEY AUTOINCREMENT,
            token TEXT UNIQUE NOT NULL,
            tenant_id INTEGER NOT NULL,
            status TEXT DEFAULT 'pending',
...
# ─── Azure One-Click Setup Token helpers ─────────────────────────────────────

def create_azure_setup_token(tenant_id: int) -> str:
    import uuid as _uuid
    from datetime import timedelta
    token = str(_uuid.uuid4())
    expires = (datetime.utcnow() + timedelta(minutes=30)).isoformat()
    conn = get_db()
    conn.execute(
        "INSERT INTO azure_setup_tokens (token, tenant_id, expires_at) VALUES (?, ?, ?)",
        (token, tenant_id, expires)
    )
    conn.commit()
    conn.close()
...
def validate_azure_setup_token(token: str) -> dict:
    """Returns token row if valid (not expired, not used), else None."""
    conn = get_db()
    row = conn.execute(
        "SELECT * FROM azure_setup_tokens WHERE token=?", (token,)
    ).fetchone()
    conn.close()
    if not row:
        return None
...
def consume_azure_setup_token(token: str, provider_id: str, status: str = "connected"):
    conn = get_db()
    conn.execute(
        "UPDATE azure_setup_tokens SET used=1, status=?, provider_id=? WHERE token=?",
        (status, provider_id, token)
    )
    conn.commit()
    conn.close()
...
def get_azure_setup_token_status(token: str) -> dict:
    conn = get_db()
    row = conn.execute(
        "SELECT status, provider_id, used, expires_at FROM azure_setup_tokens WHERE token=?",
        (token,)
    ).fetchone()
    conn.close()
    if not row:
...
# ─── GCP One-Click Setup Token helpers ───────────────────────────────────────

def create_gcp_setup_token(tenant_id: int) -> str:
    import uuid as _uuid
    from datetime import timedelta
    token = str(_uuid.uuid4())
    expires = (datetime.utcnow() + timedelta(minutes=30)).isoformat()
    conn = get_db()
    conn.execute(
        "INSERT INTO gcp_setup_tokens (token, tenant_id, expires_at) VALUES (?, ?, ?)",
        (token, tenant_id, expires)
    )
    conn.commit()
    conn.close()
...
def validate_gcp_setup_token(token: str) -> dict:
    """Returns token row if valid (not expired, not used), else None."""
    conn = get_db()
    row = conn.execute(
        "SELECT * FROM gcp_setup_tokens WHERE token=?", (token,)
    ).fetchone()
    conn.close()
    if not row:
        return None
...
def consume_gcp_setup_token(token: str, provider_id: str, status: str = "connected"):
    conn = get_db()
    conn.execute(
        "UPDATE gcp_setup_tokens SET used=1, status=?, provider_id=? WHERE token=?",
        (status, provider_id, token)
    )
    conn.commit()
    conn.close()
...
def get_gcp_setup_token_status(token: str) -> dict:
    conn = get_db()
    row = conn.execute(
        "SELECT status, provider_id, used, expires_at FROM gcp_setup_tokens WHERE token=?",
        (token,)
    ).fetchone()
    conn.close()
    if not row:
